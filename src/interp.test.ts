...
function makeLut(): Lut {
  ...
  return { mode: 'oklch', lMax: 1, cmax: 1, lSteps, hSteps, data };
}
...
describe('maxChroma', () => {
  ...
  it('returns 0 at the lightness extremes', () => {
    expect(maxChroma(lut, 0, 90)).toBeCloseTo(0, 6);
    expect(maxChroma(lut, 1, 90)).toBeCloseTo(0, 6);
  });

  it('reads grid nodes exactly on the mid row', () => {
    // hSteps=4 -> hue step = 90deg. hue 90 -> hi=1 -> 16384/65535
    expect(maxChroma(lut, 0.5, 90)).toBeCloseTo(16384 / 65535, 5);
    expect(maxChroma(lut, 0.5, 180)).toBeCloseTo(32768 / 65535, 5);
  });

  it('interpolates between lightness rows', () => {
    // L=0.25 is halfway between row0 (all 0) and row1; hue 270 -> node 65535/65535=1
    expect(maxChroma(lut, 0.25, 270)).toBeCloseTo(0.5, 5);
  });

  it('wraps hue across the 360/0 seam', () => {
    // hue 315 sits halfway between hi=3 (val 1.0) and hi=0 (val 0.0, wrapped)
    expect(maxChroma(lut, 0.5, 315)).toBeCloseTo(0.5, 5);
    // hue 360 == hue 0
    expect(maxChroma(lut, 0.5, 360)).toBeCloseTo(maxChroma(lut, 0.5, 0), 6);
  });

  it('clamps lightness outside the range', () => {
    expect(maxChroma(lut, -5, 90)).toBeCloseTo(0, 6);
    expect(maxChroma(lut, 99, 90)).toBeCloseTo(0, 6);
  });
});
