...
describe('generated LUTs', () => {
  it('oklchSrgb is self-describing with zeroed L extremes', () => {
    expect(oklchSrgb.mode).toBe('oklch');
    expect(oklchSrgb.lMax).toBe(1);
    expect(oklchSrgb.data.length).toBe(65 * 256);
    ...
    for (let hi = 0; hi < 256; hi++) {
      ...
      expect(oklchSrgb.data[64 * 256 + hi]).toBe(0);
    }
    // a mid-lightness cell is non-zero
    expect(oklchSrgb.data[32 * 256 + 0]).toBeGreaterThan(0);
  });

  it('lchSrgb carries the CIE scale (mode lch, lMax 100)', () => {
    expect(lchSrgb.mode).toBe('lch');
    expect(lchSrgb.lMax).toBe(100);
    expect(lchSrgb.data.length).toBe(65 * 256);
